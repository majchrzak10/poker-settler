  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-gradient-to-br from-green-900/60 to-black/60 border border-green-800/60 rounded-2xl p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-3xl font-bold text-white mt-1 tabular-nums">
            {formatPln(totalPot)} <span className="text-base text-green-200/55 font-normal">PLN</span>
          </p>
        </div>
      ...
      <div className="bg-black/30 border border-green-900 rounded-2xl p-4">
        <label className="text-xs text-green-200/60 uppercase tracking-wider block mb-2">
          Domyślny Buy-in
        </label>
        ...
        <label className="mt-3 flex items-center justify-between gap-3 text-xs text-green-200/65">
          ...
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => onToggleAutoAddMe?.(e.target.checked)}
        ...
        {sessionPlayers.length === 0 ? (
          ...
          sessionPlayers.map(sp => {
            ...
            return (
              <div
                key={sp.playerId}
                className="bg-black/30 border border-green-900 rounded-2xl p-4 space-y-3"
              >
                <div className="flex items-center gap-3">
